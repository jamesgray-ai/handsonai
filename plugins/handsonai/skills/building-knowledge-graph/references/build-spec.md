...
- Provenance: `sources` is a list; each entry has a required `resource` (a URL, a bundle-relative path, a relative path, or a plain descriptor), an optional stable `id`, and an optional `title`. A claim in the body is attributed with a footnote whose label equals a `sources[].id`: `…sharded daily.[^acme-msa]` and `[^acme-msa]: Master services agreement`.
