...
jtbd: "When I'm building a new feature, I want a structured AI-assisted lifecycle so I go from idea to shipped code without losing context."
primitives: [coding]
building_blocks: [skill, agent, prompt]
question: "How do I build features end-to-end with an AI coding agent without losing context?"
short_answer: "Follow a 7-step lifecycle — Discover, Define, Plan, Implement, Verify, Review, Ship — where each step has a dedicated skill or agent and the artifacts (Vision Brief, PRD, plan) flow forward to keep context durable across sessions."
status: published
...
**Short answer:** Follow a 7-step lifecycle — Discover, Define, Plan, Implement, Verify, Review, Ship — where each step has a dedicated skill or agent and the artifacts (Vision Brief, PRD, plan) flow forward to keep context durable across sessions.
...
|--------|-----------------|
| `handsonai@handsonai` | Vision Brief and PRD skills (steps 0-1) |
...
|------|-------------|------------------------|--------|
| **0. Discover** | Capture a fuzzy idea as a Vision Brief, then break it into epics and features you can build one at a time | `/handsonai:writing-vision-briefs` | `agentic-coding` (Hands-on AI) |
| **1. Define** | Turn the Vision Brief (or a new idea) into a PRD with user stories, acceptance criteria, and a GitHub issue to track the feature | `/handsonai:writing-feature-prds` | `agentic-coding` (Hands-on AI) |
...
**Command:** `/handsonai:writing-vision-briefs`
...
4. **Handoff** — Claude saves the brief (with the feature breakdown) to `specs/[name]-vision.md` and tells you to run `/handsonai:writing-feature-prds` for your first feature
...
**Next step:** Run `/handsonai:writing-feature-prds` for your first feature and tell Claude which feature from the breakdown you're starting with. Claude will pull context from the Vision Brief and scope the PRD to that one feature — so you go from idea to spec without repeating yourself.
...
**Command:** `/handsonai:writing-feature-prds`
...
**When to use it:** Use this when you know which feature you're building and need a detailed spec. If you're coming from a Vision Brief, pick one feature from your breakdown and spec it out here. If you're starting fresh with a small, focused idea, you can start here directly — but if the idea is bigger than one feature, Claude will suggest running [`/handsonai:writing-vision-briefs`](#writing-vision-briefs) first to break it down.
...
The two agentic-coding skills ship inside the Hands-on AI plugin. On Claude (Chat or Cowork) or ChatGPT, add the `jamesgray-ai/handsonai-plugins` marketplace from the Plugins page and install the plugin; in Claude Code:
...
See [Set Up the Skills](/ai-workflow-framework/skills/) for every platform, and [Using Plugins](/use-the-playbook/build/using-plugins/) for plugin management in Claude Code.
  </TabItem>
  <TabItem label="Any Platform (ZIPs)">

Download `writing-vision-briefs.zip` and `writing-feature-prds.zip` from the [GitHub Releases page](https://github.com/jamesgray-ai/handsonai-plugins/releases/latest) and add them to your platform's skill directory, or upload them where your platform has a skill upload.

Works with **Claude, ChatGPT, Cursor, Codex, Gemini CLI, VS Code Copilot**, and more. See [How to Add Skills to Your Platform](../../../agentic-building-blocks/skills/#how-to-install-skills) for each tool.
  </TabItem>
</Tabs>
