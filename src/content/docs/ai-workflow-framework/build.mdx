...
1. **Load Design Spec** — Read the approved spec from `outputs/`.
...
3. **Build path choice** — Choose "I'll build it" (the model generates artifacts) or "I'll build it myself" (get a Construction Guide with build sequence and creation skill recommendations).
4. **Mechanism-specific build path** — Only the steps relevant to your chosen orchestration mechanism (Prompt, Skill-Powered Prompt, or Agent).
5. **Discover creation tools** — Scan your environment for skills that can create other building blocks (e.g., skill-creator, agent-development). Present a Creation Tools Map for confirmation.
6. **Integration research** — Web search to verify platform availability for every tool in the spec.
7. **Generate platform artifacts** — For each building block, either delegate to the matched creation skill or generate inline using format specifications.
...
**Command:** `/handsonai:build` (Claude Code) — or invoke by name on any other platform.
