...
1. **Load Design Spec and Workflow Requirements** — Read both approved documents from `outputs/[name]/`.
2. **Build path choice** — Choose "Claude builds it" (the model generates artifacts) or "You build it yourself" (get a Construction Guide with build sequence and creation skill recommendations).
3. **Mechanism-specific build path** — Only the steps relevant to your chosen orchestration mechanism (Skill or Agent). This is where context gets prepared — the Context Inventory and Data Readiness Summary are resolved before dependent steps run (see "Prepare Context" below).
4. **Discover creation tools** — Scan your environment for skills that can create other building blocks (e.g., skill-creator). Present a Creation Tools Map for confirmation. Then resolve platform format requirements from the platform registry.
5. **Check for existing skills** — Incorporate any skills or prompt instructions you already built for this workflow.
6. **Integration research** — Verify platform availability and setup for every tool in the spec, including write-access and least-privilege pre-flights.
7. **Generate platform artifacts** — For each building block, either delegate to the matched creation skill or generate inline using format specifications, then place each artifact per the Deployment Plan.
...
- **Find existing documents** — Locate the style guides, templates, reference materials, and example files your workflow needs. These may live in shared drives, a CRM, or your local machine.
...
1. **"Claude builds it"** — The model generates all artifacts (skills, agents, configs) based on the approved spec and places them where they belong. This is the recommended default.
2. **"You build it yourself"** — The model provides a Construction Guide with build sequence, format references, and pointers to creation skills you can invoke — but you do the building.
...
1. **Prepare context** — Build the context artifacts listed in your Workflow Requirements' Context Inventory
...
| File | What it is |
|------|-----------|
| Platform artifacts | Skills, agents, and configs generated in whatever format your platform needs |
