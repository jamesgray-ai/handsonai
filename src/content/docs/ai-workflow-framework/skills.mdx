...
:::tip[Recommended: use Claude Cowork for this course]
The course is best done in **Claude Cowork** (the **Cowork** tab in the Claude Desktop app), not Claude Chat in the browser. Cowork has built-in plugin management — one install gets you all seven skills plus the `framework-agent` orchestrator that runs the whole framework end-to-end.

If your company blocks personal plugins in Cowork, or you only have a browser available, the **Claude Chat** instructions below still work — but you'll upload skills one at a time, and the `framework-agent` orchestrator isn't available in Chat (it's a plugin-only component).
:::
...
**How sharing works:** Skills you upload here in Claude Chat are also available in **Claude Cowork** automatically — they share the same personal skill library on your account. It doesn't work the other way around: skills bundled inside Cowork **plugins** (like the `handsonai` plugin) only run in Cowork, not in Claude Chat. So if you want the same skills in both surfaces, upload them here.
...
To refresh a skill later, see [Update the Skills → Claude Chat](#update-the-skills) — re-uploading replaces the existing version.
...
:::note[How Chat and Cowork share skills]
Installing the `handsonai` plugin here in Cowork makes the skills **and** the `framework-agent` orchestrator available in **Cowork only** — plugin-installed skills don't show up in Claude Chat in the browser.

If you also want the seven skills in **Claude Chat**, upload them via **claude.ai → Customize → Skills → + → Create skill → Upload skill** (see the Claude Chat section above). Skills uploaded that way appear in both Chat and Cowork on the same account. The `framework-agent` orchestrator is plugin-only, so it stays Cowork-only either way.

**Personal plugins blocked at your company?** Some organizations disable personal plugins in Cowork. If you don't see the **Personal** tab in **Browse plugins**, use the **Claude Chat** instructions above instead — uploading skills via Customize → Skills doesn't require plugin permissions.
:::
...
Some organizations disable personal plugins. If you can't add a marketplace or install plugins in the Code tab, use the **Claude Chat (claude.ai)** instructions above to upload skill `.zip` files via **Customize → Skills → + → Create skill → Upload skill** — that path makes the skills available in Claude Chat and Cowork on the same account, which covers most of the framework workflow. (Slash commands like `/handsonai:analyze` are specific to the Code tab and require the plugin install, and the `framework-agent` orchestrator is also plugin-only — it won't be available via the upload path.)
:::
...
1. Re-download the latest ZIP from [GitHub Releases](https://github.com/jamesgray-ai/handsonai-plugins/releases/latest).
2. In Claude.ai, click **Customize** in the left sidebar → **Skills** → **+** → **Create skill** → **Upload skill**, and select the new `.zip`. It replaces the existing skill of the same name.
...
**Native Skills:** Re-download the latest ZIP from [GitHub Releases](https://github.com/jamesgray-ai/handsonai-plugins/releases/latest) and upload it again — re-uploading replaces the existing skill of the same name.
