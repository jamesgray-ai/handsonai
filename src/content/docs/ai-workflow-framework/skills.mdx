...
Some organizations disable personal plugins in Cowork. If you don't see the **Personal** tab in **Browse plugins**, or your admin has restricted plugin installs, use the **Claude Chat (claude.ai)** instructions in the section above instead — download the skill `.zip` files from GitHub Releases and upload them via **Customize → Skills → + → Create skill → Upload skill**. Skills uploaded this way are shared across Claude Chat and Cowork on the same account, so they'll show up in Cowork automatically.
:::
...
:::note[Personal plugins blocked at your company?]
Some organizations disable personal plugins. If you can't add a marketplace or install plugins in the Code tab, use the **Claude Chat (claude.ai)** instructions above to upload skill `.zip` files via **Customize → Skills → + → Create skill → Upload skill** — that path makes the skills available in Claude Chat and Cowork on the same account, which covers most of the framework workflow. (Slash commands like `/handsonai:analyze` are specific to the Code tab and require the plugin install.)
:::
