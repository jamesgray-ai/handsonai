...
outputs/articles/<slug>/
  ...
  APPROVED                written when you approve; the publish gate checks for it
  run-log.md              audit trail, one line per stage, appended by the gate
```
...
| Draft length | the draft is under the floor **or over the ceiling** — the first live run overran the target by 21% because only a floor existed |
| Clean output | the article contains stray wrapper tags like `</content>`, which agents occasionally emit from their own scaffolding |
...
**This is the goal statement for the whole multi-agent system.** It is the only thing you type. Two paragraphs start four agents, produce nine files, and deliver two documents.
...
The first builds synthetic workspaces and asserts the `SubagentStop` gate blocks a thin dossier, an uncited dossier, a draft that is too short or too long, a revision missing its memo, a critique masquerading as a revision, stray wrapper tags in the article, a missing Word file, and a corrupt Word file — and that it stays completely inert when the pipeline is not running.
...
Everything above describes how the pipeline is designed. This is what it did the first time it ran end to end, taken from the artifacts it left behind — because a worked example that only describes itself is a brochure.

**Twelve minutes, four agents, no intervention** beyond one approval. The dispatch log the `PreToolUse` hook wrote:

```
14:01:38 tool=Agent target=ai-productivity-researcher
14:06:20 tool=Agent target=tech-executive-writer
14:08:17 tool=Agent target=hbr-editor
14:11:38 tool=Agent target=hbr-publisher
```

Nobody specified that order. Claude chose it by reading the agents' descriptions.

**What the artifacts show:**

| Measure | Result |
|---|---|
| Research dossier | 4,009 words, 36 cited sources, five primary deployments |
| Citations through the chain | 36 → 53 → 54 → 54 — the writer added inline links, the editor *closed a gap*, nothing was lost |
| Critique leaking into the article | zero |
| Final deliverables | 3,030-word markdown article, valid 8-page Word document |
| Approval marker written | 14:11:27 — eleven seconds before the publisher was dispatched |

The article opened on a genuine hook — 95% of AI pilots return nothing, per MIT's Project NANDA — and named seven organisations with quantified outcomes: Klarna, Moderna, JPMorgan Chase, Sierra, BT Group, WeightWatchers, and Sonos. The detail worth pointing at is that it flagged every company-reported figure as self-reported rather than independently audited. That scepticism came from the researcher's instructions, not from the prompt.

**Two things the run got wrong, which the gate now catches:**

1. **The article ran 3,030 words against a 2,000–2,500 target.** The gate had a floor but no ceiling, so nothing objected. Adding a ceiling of 2,750 words means the overrun is now caught at the *draft* stage, before it propagates through editing and publishing.
2. **The editor left a literal `</content>` tag** at the end of its file — scaffolding from its own output format. The publisher happened to strip it, so the deliverable was clean by luck. The gate now rejects stray wrapper tags outright.

Both are the same lesson: **a check you did not write is a standard you are not enforcing.** The pipeline looked like it worked, and the failures were only visible by measuring the artifacts afterwards.

**One thing the run did not prove.** No gate ever blocked — every agent cleared the bars first time, and Claude asked for approval before attempting to publish rather than being stopped. So the block paths are verified by 41 unit assertions, not by this run. If you want an audience to *see* the harness stop the AI, force it: arm a workspace and ask Claude to dispatch `hbr-publisher` with no approval marker present.
...
| The first live run overran the word target by 21%, and nothing objected — the gate had a floor but no ceiling | Bars you state only in the prompt are not enforced. Every standard worth having needs a check, and a one-sided check is half a standard. |
| The editor emitted a stray `</content>` tag; the deliverable was clean only because the publisher happened to strip it | Agents leak their own scaffolding into their output. Check the shape of the artifact, not just its substance. |
| The run log recorded ten entries for four stages, because `SubagentStop` fires more than once per agent | An audit trail nobody can read is not an audit trail. Log state *changes*, not events. |
