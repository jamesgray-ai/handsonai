...
**Construct (Step 3.2) now discovers creation tools automatically.** The `constructing-workflows` skill no longer hardcodes which tools to use for building skills and agents. Instead, it [scans your environment at runtime](../../ai-workflow-framework/build/#how-creation-tools-are-discovered) to find whatever creation skills you have installed — then delegates to them or generates inline. Works across Claude Code, Cursor, Codex CLI, Gemini CLI, and VS Code Copilot. Business-First AI Framework plugin updated to v6.0.3.
