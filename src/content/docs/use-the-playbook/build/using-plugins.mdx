...
:::note
The AI Registry skills work best with the [Notion MCP connector](../../../builder-setup/notion-registry-setup/) configured. Without it, Claude can follow the naming conventions and draft the output but cannot read from or write to Notion.
:::
...
:::note
MCP-dependent skills (like `registering-building-blocks`) won't work via the raw API since they require tool integrations that the API alone doesn't provide.
:::
...
2. Check the [compatibility table](#skill-compatibility-across-platforms) — some skills work best with Notion MCP
